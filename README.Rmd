...
<!-- badges: start -->
  [![Launch Rstudio Binder](http://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/rivm-syso/escape2024/main?urlpath=rstudio)
  <!-- badges: end -->
...
As a fraction of the total population, the fraction ultimately infected is `r final_z |> round(4)`.
...
where the desired accuracy is machine precision: `r .Machine$double.eps`. Note the correspondence in theoretical and simulated final size: `r theory_final_z` vs `r final_z`. 
...
The theoretical reproduction number is `r reproduction_theory |> round(2)` and the theoretical final size is `r final_size_network_theory$final |> round(4)` (compare to the simulated final size `r z |> round(4)`).
